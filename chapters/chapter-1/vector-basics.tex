\chapter{Vector Basics}
    ...
    \section{Coordinate Systems}
        ...
        \subsection{Cylindrical Coordinate System}
            ...
            A point in CCS, as depicted in figure~\ref{fig:pointinccs}, is represented by three coordinates $\left(\rho, \phi, z\right)$. $\rho$ is the distance between the origin and the point's projection on the $xy$ plane. $\phi$ is the angle between the x-axis and the $\rho$ line. $z$ is the distance between the point and it's projection on the $xy$ plane. The three unit vectors in this coordinate system are $\vec{a}_\rho$, $\vec{a}_\phi$ and $\vec{a}_z$. $\vec{a}_z$ is the same as in RCS. $\vec{a}_\rho$ takes the direction along the $\rho$ line, and $\vec{a}_\phi$ takes the direction of increasing $\phi$.
